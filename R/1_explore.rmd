...
Here is a brief summary of the samples collected from various trials being run at Ocean Era along with source water for the tanks and limu collected from the fish cages.

```{r}
# summarize each experiment

oce_summary = oce_era[ , .("Sample Count" = .N), by = .( experiment, description, type)]

datatable(oce_summary,
          options = list(pageLength = 10))
```
...
limu_mat = prep_ht_mat(otu = limu_otu,
           tax_table = tax,
           tax_level = "family",
           cutoff = 40,
           scale_vals = T)
...
ha = HeatmapAnnotation(
  ...
  col = list(genus = color_pallete[unique(genera)],
             nutrient = color_pallete[unique(nutrients)]),
...
# get family summed matrix
water_mat = prep_ht_mat(otu = water_otu,
           tax_table = tax,
           tax_level = "family",
           cutoff = 40,
           scale_vals = T)
